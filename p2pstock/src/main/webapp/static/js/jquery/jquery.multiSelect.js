...
//	
*/
function multiSelectValue(ptrArray,ptrInputIndex,ptrCheckBoxIndex,multiInput){
	var parray = ptrArray.split(",");
	var m = parray.length;
	var n = ptrInputIndex - 1;
	if(!(m==n)){
		return;
	}
	for(var i=1;i<ptrInputIndex;i++){
		var par = parray[i-1];
		var value1 = $("#"+par).val();
		$("#"+multiInput+i).attr("value",value1);
		//checkBox处理
		var ary = value1.split(",");
		for(var j=0;j<ary.length;j++){
			for(var i=1;i<checkBoxIndex;i++){
				if($("#"+checkBox+i).attr("value") == ary[j]){
    ...
    $.extend($.fn, {
        multiSelect: function(o, callback) {
            ...
            $(this).each(function() {
                var select = $(this);
                var iWidth = 150;
                var html = '<div class="multiSelect"><input type="text" class="multiSelect_txt" readonly="readonly" style="width:150px"  value="" id="'+ multiInput + multiInputIndex + '"/></div>';
                ...
                $(select).find('OPTION').each(function() {
                    if ($(this).val() != '') {
                        html += '<label><input type="checkbox" id="'+checkBox+checkBoxIndex+'"name="' + $(select).attr('name') + '" value="' + $(this).val() + '"';
                ...
                $(select).next('.multiSelect').next('.multiSelectOptions').find('INPUT:checkbox').click(function() {
                    ...
                    display = display.substr(0, display.length );
