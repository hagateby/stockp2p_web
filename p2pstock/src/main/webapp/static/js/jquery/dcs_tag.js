...
//var gDomain="sdc.sinosig.com"; // SDC Production Mode Domain
//var gDcsId="dcsw88etf1000008a94gez3kw_4z7h";

var gDomain="10.63.65.43:9090"; // SDC Production Mode Domain
var gDcsId="log-server";
var gFpc="WT_FPC";
var gConvert=true;
...
function dcsVar(){
	...
	//navigator 浏览器对象 包含浏览器相关信息 兼容浏览器
	//WT.ul 操作系统设定的自然语言
		...
		for (var i=0;i<length;i++){
			...
			if (name.length>0){
				if (name.indexOf("WT.")==0){
					...
					//增加该段落将WT.cg_ygbx(如"首页/一级栏目/二级栏目...")分解到WT.cg_n, WT.cg_s, WT.cg_3...中 
					if (name.indexOf("WT.cg_ygbx")==0) {
						encode=true;
						var cg_ygbx_list=content.split("/");
						for (var ii=0;ii<cg_ygbx_list.length;ii++){
							if (ii==0) {
								WT["cg_n"]=encode?dcsEscape(dcsEncode(cg_ygbx_list[ii]),I18NRE):cg_ygbx_list[ii];
							} else if (ii==1) {
								WT["cg_s"]=encode?dcsEscape(dcsEncode(cg_ygbx_list[ii]),I18NRE):cg_ygbx_list[ii];
							} else {
								WT["cg_"+(ii+1)]=encode?dcsEscape(dcsEncode(cg_ygbx_list[ii]),I18NRE):cg_ygbx_list[ii];
							}
						}
						delete WT.cg_ygbx;
					}
...
function dcsTag(){
	...
	var P="http"+(window.location.protocol.indexOf('https:')==0?'':'')+"://"+gDomain+(gDcsId==""?'':'/'+gDcsId)+"/servlet/ClientInfoGetServlet?";
	for (var N in DCS){
		if (DCS[N]) {
			P+=dcsA(N,DCS[N]);
		}
