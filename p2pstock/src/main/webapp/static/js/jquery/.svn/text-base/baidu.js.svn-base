(function() {
	...
	A.ui.suggestion._Div = function(V, o, Z, a, X) {
		var d = this, T, Y, l, f = X.class_prefix, U, S = document
				.createElement("DIV");
		...
		function g(C) {
			var B = T && (typeof C == "number" && typeof T[C] != "undefined") ? T[C]
					: C;
			V.call("pick_word", B);
		}
		function W() {
	...
	A.ui.suggestion._Suggestion = function(F, B) {
		...
		E._adjustPos = function(S) {
			var J = G.element, M = G.shade, P = document, N = P.compatMode == "BackCompat" ? P.body
			...
			if (A.ie && document.compatMode == "BackCompat") {
				...
				var U = parseFloat(A.getStyle(J, "borderLeftWidth")) || 0, W = parseFloat(A
						.getStyle(J, "borderRightWidth")) || 0, T = parseFloat(A
						.getStyle(J, "marginRight")) || 0;
...
var BaiduSuggestion = (function() {
	...
	function I(T, Z, S, X) {
		...
		if (Z == null) {
			...
			L(".bdSug_wpr", "border:1px solid " + Y
					+ ";position:absolute;z-index:9;top:28px;left:0;color:" + Q);
