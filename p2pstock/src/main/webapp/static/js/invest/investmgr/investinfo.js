...
function startnoBlur(obj,price,sales_city){
	...
	if(sub_code != '' ){
		$(obj).parent().next('td').next('td').html(all.toFixed(2)+'元')
	}
...
function subcodeBlur(obj,price,sales_city){
	...
	if(start_no != ''){
		$(obj).parent().next('td').html(all.toFixed(2)+'元')
	}
}
function del(obj){
...
function ok(obj,price,invester_bail,user_accruedcharges_amount,user_accruedcharges_type,basic_product_id,sales_city,up_limit,end_date){
	
	//验证是否过期
	var nowdate = getCurrentDate2();
	if(dateCompare(nowdate,end_date) == "1"){
		$('#lerror').text('该基础标的已经过期，不能投资');
		return;
	}else{
		$('#lerror').text('');
	}
	...
	if(up_limit != '0' && up_limit != ''){
		if(sub_code*feecnt > up_limit){
			$('#lerror').text('该基础标的申购上限为:' + up_limit +"股");
			return;
		}else{
			$('#lerror').text('');
		}
...
function addproduct(sales_cityname,basic_product_name,invest_product_id,price,invester_bail,user_accruedcharges_type,user_accruedcharges_amount,basic_product_id,sales_city,up_limit,end_date){
	var uamountlbl;
	...
	+ "<td class='tbl_td_20'>" + end_date + "</td>" 
	...
	+ "," + basic_product_id + ","+sales_city + "," + up_limit + "," +'"'+end_date+'"'+")'>确定</a></td></tr>";
